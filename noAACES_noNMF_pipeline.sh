#!/bin/bash
exec &> noAACES_noNMF_analysis.out
...
#Rscript 1.DataInclusion/Scripts/A.getInclusion.R
...
Rscript 2.Clustering_DiffExprs/Scripts/D.NMF.R $KMIN $KMAX $NSTARTSNMF $SEED $DATASETS "GSE26712_eset"
echo "FINISHED: NMF step 1"

# Run SAM on NMF clusters (TRUE argument forces NMF analysis)
echo "RUNNING: NMF step 2"
Rscript 2.Clustering_DiffExprs/Scripts/A.run_kmeans_SAM.R $KMIN $KMAX $NSTARTSNMF $SEED TRUE $NO_SHUFFLE $SAM_SUBSET \
$DATASETS "GSE26712_eset"
